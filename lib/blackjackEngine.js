...
async function playGame(interaction, { transition = 'update', bet = 0, t } = {}) {
  ...
  const upRank = dealer[0].rank
  const upIsAce = upRank === 'A'
  const upIsTen = ['10','J','Q','K'].includes(upRank)
  if (upIsAce || upIsTen) {
  ...
  const finalizeDealer = () => {
    // Dealer hits on soft 17 (H17): if dealer total is less than 17 -> hit
    // if dealer total is 17 and it's a soft 17 -> hit as well
    while (true) {
  ...
  if (id === 'pdm-bj-split') {
    ...
    collector.on('end', async (collected, reason) => {
      try {
        ...
        const outcomes = []
        const naturals = []
        const dealerNatural = (playerHands && dealer && dealer.length === 2 && handValue(dealer) === 21)
        for (const hand of playerHands) {
          const pval = handValue(hand)
